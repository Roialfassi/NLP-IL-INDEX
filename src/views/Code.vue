...
export default {
  data() {
    return {
      ...
      tags: ['Tokenization', 'Morphological and Syntactic Analysis', 'Diacritization / Nikkud / Vocalization', 'Models', 'Code Other', 'Commercial services'],
      selectedTag: 'all'
    }
  },
  computed: {
  ...
  beforeMount(){
    let sites = require("@/assets/Code.json").map(url => ( {...url, expanded: false}))
    this.items = sites;
 },
  methods: {
    filterItems() {
