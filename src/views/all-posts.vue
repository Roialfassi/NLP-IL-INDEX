...
export default {
  data() {
    return {
      ...
      tags: ['Corpora', 'Code', 'Linguistic Resources', 'Papers', 'Courses, presentations and meetups', 'Tagging Tools'],
      selectedTag: 'all'
    }
  },
  computed: {
  ...
  beforeMount(){
    let sites = require("@/assets/All-Items.json").map(url => ( {...url, expanded: false}))
    this.items = sites;
 },
  methods: {
    filterItems() {
